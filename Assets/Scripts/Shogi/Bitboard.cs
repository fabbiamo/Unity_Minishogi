...
namespace Assets.Scripts.Shogi
{
    ...
    public struct Bitboard
    {
        ...
        /// <summary>
        /// 32bit構造体で初期化するコンストラクタ
        /// </summary>
        /// <param name="p_"></param>
        public Bitboard(UInt32 p_)
        {
            p = p_;
        }
        ...
        {
            return (p & SQUARE_BB[sq.ToInt()].p) != 0;
        }
        ...
        {
            int index = (int)((occupied.p >> Slide[(int)sq]) & 0x7);
            File f = sq.ToFile();
            return new Bitboard(RookFileEffectBB[(int)sq.ToRank(), index] << (int)Util.MakeSquare(f, Rank.RANK_1));
        }
        ...
        public static Bitboard PawnDropBB(int i)
        {
            return PAWN_DROP_MASK_BB[i];
        }
        ...
        private static UInt64 OccupiedToIndex(Bitboard occupied, Bitboard mask) { return BitOp.PEXT32(occupied.p, mask.p); }
        ...
        public static void Init()
        {
            ...
            EnemyFieldBB = new Bitboard[(int)Color.NB] { RANK1_BB, RANK5_BB };
...
*/
        ...
        private static Bitboard[] PAWN_DROP_MASK_BB = new Bitboard[0x20];
        ...
        /// 玉、金、銀、桂、歩の利き
        /// </summary>
        private static Bitboard[] KingEffectBB = new Bitboard[(int)Square.NB + 1];
        private static Bitboard[,] GoldEffectBB = new Bitboard[(int)Square.NB + 1, (int)Color.NB];
        private static Bitboard[,] SilverEffectBB = new Bitboard[(int)Square.NB + 1, (int)Color.NB];
        private static Bitboard[,] KnightEffectBB = new Bitboard[(int)Square.NB + 1, (int)Color.NB];
        private static Bitboard[,] PawnEffectBB = new Bitboard[(int)Square.NB + 1, (int)Color.NB];

        // 盤上の駒をないものとして扱う、遠方駒の利き。香、角、飛
        private static Bitboard[,] LanceStepEffectBB = new Bitboard[(int)Square.NB + 1, (int)Color.NB];
        private static Bitboard[] BishopStepEffectBB = new Bitboard[(int)Square.NB + 1];
        private static Bitboard[] RookStepEffectBB = new Bitboard[(int)Square.NB + 1];

        // 角の利き
        private static Bitboard[,] BishopEffectBB = new Bitboard[2, 68 + 1];
        private static Bitboard[,] BishopEffectMask = new Bitboard[2, (int)Square.NB + 1];
        private static int[,] BishopEffectIndex = new int[2, (int)Square.NB + 1];

        // 飛車の縦、横の利き

        // 飛車の縦方向の利きを求めるときに、指定した升sqの属するfileのbitをshiftし、
        // index を求める為に使用する。(from Apery)
        private static Byte[] Slide = new Byte[(int)Square.NB + 1]
        {
              1 , 1 , 1 , 1 , 1 ,
              6 , 6 , 6 , 6 , 6 ,
              11, 11, 11, 11, 11,
        ...
        private static UInt32[,] RookFileEffectBB = new UInt32[(int)Rank.NB + 1, 8];
        private static Bitboard[,] RookRankEffectBB = new Bitboard[(int)File.NB + 1, 8];
